...
### Fixed 2026-02
- ✅ **D-150** Rask kopiering av passord fra vault-listen. Kopi-ikon på hver entry-rad (Alt A) + long-press på raden (Alt C, 500ms delay + haptisk feedback på mobil). Bruker eksisterende `copyWithAutoClear` (D-113/D-017 clipboard-mønster) med `clipboardClearSeconds` fra client-config (default 120s). Skjules automatisk hvis `clipboardEnabled: false`. Ny `useLongPress`-hook (generisk, pointer-events, threshold-basert cancel). 6 nye i18n-nøkler × 4 språk. Passordet går rett fra vault-blob (klient-side dekryptert) → clipboard uten mellomsteg på skjerm. Statisk QA: TSC ✓, lint:all 7/7 ✓, build ✓.
- ✅ **D-145** Test connectivity-verktøy for B2B-parents i TenantViewer: nytt `ConnectivityTestCard` med en-klikks HEAD-request til `https://<subdomain>.kodovault.no`, viser grønt/rødt-badge med HTTP-status + responsetid. Plassert både i Oversikt-fanen og rett over `CreateOrgAdminCard` i Lisens & B2B-fanen — så Mike verifiserer at D-144 admin-host-attach fungerer FØR han oppretter SuperAdmin. Nytt endpoint `POST /api/admin/tenants/[subdomain]/test-connectivity` (8s timeout, HEAD-request, ingen state-endring, ingen PII). 10 i18n-nøkler × 4 språk (4 EXEMPT-mapped for dynamisk `status_*`-lookup). Statisk QA: TSC ✓, lint:all 7/7 ✓, build ✓.
- ✅ **D-144** Automatisk Vercel-registrering av `<prefix>-admin.kodovault.no`-hosts ved opprettelse av B2B-parents. Tidligere måtte Mike manuelt legge til hosten i Vercel Dashboard for hver ny B2B-parent, ellers feilet TLS-handshake med `ERR_CONNECTION_CLOSED`. Ny `attachAdminSubdomain()`-helper + dynamisk oppslag av hoved-prosjekt-ID (`kodo-vault`) via `GET /v9/projects` med modul-lokal cache. Failsoft ved API-feil, idempotent ved eksisterende registrering (Vercel 409). Statisk QA: TSC ✓, lint:all 7/7 ✓, build ✓.
- ✅ **D-143** Native `window.confirm()`/`alert()` i Konsoll → Team ved sletting av admin-konto: erstattet med mørk `ConfirmDialog` + type-to-confirm på e-post (samme mønster som `EmployeeListSection`-slett i D-116). Alle `alert()`-kall i `TeamManagementSection.tsx` (self-delete guard, last-super-admin guard, generisk HTTP-feil, samt suspend/unsuspend-feilmeldinger) erstattet med `toast.error`. Ny suksess-toast ved gjennomført sletting. 4 nye i18n-nøkler × 4 språk (`delete_dialog_title/desc/confirm` + `delete_success`), fjernet ubrukt `confirm_delete`. Statisk QA: TSC ✓, lint:all 7/7 ✓, build ✓.
- ✅ **D-142** Login-historikk åpner i sentrert modal i stedet for inline-ekspansjon. Kompakt 5-rads-kort er alltid synlig på Sikkerhet-fanen; "Vis full historikk"-lenken åpner et modal med filter-pills (7/30/90 dager), full scrollbar tabell og sticky header. Modalen lukkes via X-knapp, "Lukk"-footer-knapp, ESC eller backdrop-klikk. Bonus: byttet rekkefølge på Generelle-fanen (Org-info først, deretter Språk-velgerne). Ny i18n-nøkkel `am_admin_settings.history_modal_close` × 4 språk (erstatter dead `history_show_less`). Statisk QA: TSC ✓, lint:all 7/7 ✓, build ✓.
- ✅ **D-141** Per-org fakturahistorikk for am-admin (Konsoll → Innstillinger → Fakturering): `InvoiceHistoryCard` (D-139) refaktorert med `endpoint`-prop slik at samme UI-komponent kan brukes av både super-admin (Mike) og firma-admins (am-admin). Nytt endepunkt `GET /api/am-admin/invoices?period=30d|90d|365d|all` speiler shape fra D-139, beskyttet av `requireAmAdmin` (både super-admin og admin har tilgang — org-aggregat, ingen PII per ansatt). `stripeCustomerId` strømmes gjennom `/api/am-admin/auth/me` → `KonsoletSettingsPanel` → `KonsollBillingTab` → `InvoiceHistoryCard`. Statisk QA: TSC ✓, lint:all 7/7 ✓ (ny EXEMPT-oppføring for D-141-ruten i coverage-matrix-lint), build ✓.
- ✅ **D-140** Send faktura feilet "This invoice is already finalized, you can't re-finalize a non-draft invoice" ved gjenklikk på "Bekreft og send" samme dag: D-133 idempotency-key returnerer samme invoice fra cachen, men den er allerede finalisert fra attempt 1 (`status="open"` eller `"paid"`). Vår kode kalte alltid `finalizeInvoice` uten status-sjekk. Fiks: branch-logikk på `invoice.status` etter create — `draft` → finalize+send; `open` → hopp over finalize, send (re-send e-post); `paid/void/uncollectible` → ingen handling (idempotency gjør jobben). MVA-diagnostikk og send-guard bevart. Testing-agent iter_36 100%.
- ✅ **D-139** Per-kunde fakturahistorikk i TenantDetailCard: nytt GET `/api/admin/tenants/[subdomain]/invoices?period=30d|90d|365d|all` kaller `stripe.invoices.list` med datofilter (default 90d). Ny `InvoiceHistoryCard.tsx` viser tabell (nummer, dato, status-badge, beløp, MVA, betalt-tidspunkt, PDF + hosted-link) + sum-rad i tfoot med total fakturert / total MVA / total betalt for valgt periode. Voided/uncollectible ekskluderes fra sum-amount/tax men inkluderes i count. Wired inn i "Stripe & Fakturaer"-tab for alle tenants med stripeCustomerId (B2C + B2B parents). Testing-agent iter_35 100%.
- ✅ **D-138** Real-world payload-matrise for webhook-event-handlers: utvidet `find-subdomain-event.test.ts` med eksplisitte assertions for 6 invoice.*-event-typer (paid, payment_failed, finalization_failed, marked_uncollectible, sent, voided) — alle bruker identisk metadata-shape som Mikes manuelle send-invoice-route. Verifiserer at D-137-fiksen treffer hele invoice-livssyklusen for D-080 manuell-faktura-flyten. customer.subscription.* utelatt fordi de strukturelt ikke kan treffe denne pathen. 15/15 PASS. Testing-agent iter_34 100%. Ingen kode-endring i event-handlers.ts — helperen var allerede payload-shape-drevet (ikke event-type-drevet).
