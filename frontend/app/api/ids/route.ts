...
// Master-passordet er aldri sendt hit. ID-blob bruker SAMME master-passord
// som hovedvault, men ulik salt — derfor egen Upstash-key (D-002/D-033).
//
// Single-user mode: én fast Redis-key (`vault:default:ids`).
// Upstash-credentials kommer fra env (KV_REST_API_URL + KV_REST_API_TOKEN).
// Mønster speiler /app/frontend/app/api/cards/route.ts.
//
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Kunne ikke hente ID-er fra server", detail: msg },
      { status: 500 },
    );
  }
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : "Ukjent feil";
    return NextResponse.json(
      { error: "Kunne ikke lagre ID-er", detail: msg },
      { status: 500 },
    );
  }
}
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : "Ukjent feil";
    return NextResponse.json(
      { error: "Kunne ikke slette ID-blob", detail: msg },
      { status: 500 },
    );
  }
}
